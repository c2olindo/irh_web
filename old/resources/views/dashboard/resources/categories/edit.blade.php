   ...
   <div class="col-lg-6 col-md-6 col-sm-12 mb-4">
    <div class="card">
      <div class="card-header border-bottom">
          <h6 class="m-0">Edit Existing Resource Category</h6>
      </div>
      <div class="card-body p-3 d-flex">
        <form action="{{ route('dashboard.resources.categories.update',$category) }}" method="POST" class="w-100">
          @csrf
          <div class="form-group">
            <label for="size_title">Title: *</label>
            <input type="text" name="title" value="{{ $category->title }}" class="form-control {{ $errors->has('title') ? ' is-invalid' : '' }}" required>
            @if ($errors->has('title'))
          ...
          <div class="form-group">
            <input type="submit" class="btn btn-primary" value=" + Update Resource Category">
          </div>
        </form>
      </div>
    </div>
