  ...
  <div class="col-lg col-md-12 col-sm-12 mb-4">
    <div class="card">
      <div class="card-header border-bottom">
        <h6 class="m-0">Add New Testimonial - <a href="{{ route('dashboard.testimonials.index') }}" class="btn btn-primary">Show All Testimonials</a></h6>
      </div>
      <div class="card-body d-flex p-5">
        <form class="col-lg-12" action="{{ route('dashboard.testimonials.store') }}" method="POST">
          @csrf
          <div class="form-group {{ $errors->has('testimonial_by') ? ' is-invalid' : '' }}">
            <label for="testimonial_by">Testimonial By</label>
            <input id="testimonial_by" type="text" class="form-control {{ $errors->has('testimonial_by') ? ' is-invalid' : '' }}" name="testimonial_by" value="{{ old('testimonial_by') }}" required autofocus>
          ...
          <div class="form-group {{ $errors->has('testimonial_text') ? ' is-invalid' : '' }}">
            ...
            <textarea name="testimonial_text" id="" rows="3" class="form-control {{ $errors->has('testimonial_text') ? ' is-invalid' : '' }}" required>{{ old('testimonial_text') }}</textarea>
